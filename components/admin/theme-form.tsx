...
import {
  ...
  updateTheme,
  type ThemeFormData,
} from "@/lib/services/admin-themes";
...
const themeSchema = z.object({
  ...
  color: z.string().regex(/^#[0-9a-fA-F]{6}$/, "Must be hex color (#RRGGBB)").nullable(),
...
export function ThemeForm({ mode, themeId, defaultValues }: ThemeFormProps) {
  ...
  const onSubmit = async (data: ThemeFormData) => {
    ...
    try {
      ...
      const normalized: ThemeFormData = {
        ...data,
      ...
      if (mode === "create") {
        await createTheme(normalized);
      } else {
        await updateTheme(themeId!, normalized);
      }
      ...
      <Tabs defaultValue="en">
        ...
        <TabsContent value="en" className="space-y-4 mt-4">
          ...
          <div>
            ...
            <Textarea id="intro_text_en" {...register("intro_text_en")} rows={8} placeholder="Markdown supported" />
          </div>
        </TabsContent>
        ...
        <TabsContent value="es" className="space-y-4 mt-4">
          <div>
            ...
            {errors.title_es && (
              <p className="text-sm text-red-500 mt-1">{errors.title_es.message}</p>
            )}
          </div>
          <div>
          ...
          <div>
            ...
            <Textarea id="intro_text_es" {...register("intro_text_es")} rows={8} placeholder="Markdown supported" />
          </div>
        </TabsContent>
      </Tabs>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <Input id="color" {...register("color")} placeholder="#a78bfa" className="flex-1" />
