...
export async function suspendQuestion(userId: string, questionId: string, reason?: string) {
...
export async function hideTheme(userId: string, themeId: string) {
  const supabase = await createClient();
...
export async function unhideTheme(userId: string, themeId: string) {
  const supabase = await createClient();
  const { error } = await supabase
    .from('hidden_themes')
    .delete()
...
export async function getHiddenThemes(userId: string) {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from('hidden_themes')
    .select(`
...
export async function updateReadingProgress(
  ...
  }, { onConflict: 'user_id,theme_id,category_id' });
  if (error) throw new Error(error.message);
}
